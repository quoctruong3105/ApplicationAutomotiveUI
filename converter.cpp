...
void converter::pushData(QString name, QString age, QString phone)
{
    QMap<QString, QList<QVariant>> newMap;
    QString path = "test.txt";
    test* t = new test();
    t = readJson(path);
    newMap = t->map();
    ...
    list.append(age);
    list.append(phone);
    newMap.insert("user " + QString::number(converter::length()), list);
    t->setMap(newMap);
